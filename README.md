...
#### 💰 AI 费用追踪
精确掌控每一分 AI 使用成本！实时统计 Token 使用量和费用，支持多维度分析（按项目、引擎、模型），可视化费用趋势图让成本一目了然。

#### 🌐 内网穿透
一键将本地项目暴露到公网！集成 Frpc 客户端，可视化配置界面，自动生成配置文件，支持进程管理和状态监控。演示项目、手机测试、团队共享都不在话下。

#### 🩺 AI 智能诊断
项目启动失败？让 AI 帮你诊断！自动收集错误日志，智能分析根本原因，提供具体可操作的修复步骤。端口占用、依赖错误统统搞定。

#### 🔌 端口管理优化
智能识别前后端端口，自动检测项目类型（frontend/backend/fullstack），支持端口冲突检测和智能推荐。前后端项目关联管理，配置更清晰。
...
- ⚡ **实时编码助手** - 流式对话，即问即答
- 💰 **费用透明可控** - 实时统计 Token 使用和费用，成本一目了然
- 🩺 **智能诊断助手** - 自动分析错误，提供解决方案
